...
cc_funct = function(space) {
  ...
  rand_cc <- sample(cc_cards_vec, size = 1, replace = FALSE)
  print(rand_cc)
  space = player_df[player, 2]
  if (rand_cc == "Go to jail. Go directly to jail, do not pass Go, do not collect $200" || rand_cc == "Advance to Go(collect $200)") {
    mono_board[space, 2] = as.numeric(mono_board[space, 2] + 1)
    mono_graph_data = rbind(mono_graph_data, c(player_df[player, 1], mono_board[player_df[player, 2], 3], mono_board[player_df[player, 2], 4], mono_board[player_df[player, 2], 1]))
    if (rand_cc == "Go to jail. Go directly to jail, do not pass Go, do not collect $200") {
      space = 11
    }
    else if(rand_cc == "Advance to Go(collect $200)") {
      space = 1
    }
  }
  else {
  ...
  r_pos = match(rand_cc, cc_cards[,1])
  assign("player_df", player_df, .GlobalEnv)
  assign("mono_board", mono_board, .GlobalEnv)
  assign("mono_graph_data", mono_graph_data, .GlobalEnv)
...
c_funct = function(space) {
  ...
  rand_c <- sample(chance_cards_vec, size = 1, replace = FALSE)
  print(rand_c)
  space = player_df[player, 2]
  if (rand_c == "Go to jail. Go directly to jail, do not pass Go, do not collect $200" || rand_c == "Advance to Boardwalk" || rand_c == "Advance to Go(collect $200)" || rand_c == "Advance to Illinois Avenue. If you pass Go, collect $200" || rand_c == "Advance to St. Charles Place. If you pass Go, collect $200" || rand_c == "Advance to the nearest Railroad. If unowned, you may buy it from the Bank. If owned, pay owner twice the rental to which they are otherwise entitled" || rand_c == "Advance token to the nearest Utility. If unowned, you may buy it from the Bank. If owned, throw dice and pay owner a total ten times amount thrown." || rand_c == "Go back 3 spaces" || rand_c == "Take a trip to Reading Railroad. If you pass Go, collect $200") {
    mono_board[space, 2] = as.numeric(mono_board[space, 2] + 1)
    mono_graph_data = rbind(mono_graph_data, c(player_df[player, 1], mono_board[player_df[player, 2], 3], mono_board[player_df[player, 2], 4], mono_board[player_df[player, 2], 1]))
    if (rand_c == "Go to jail. Go directly to jail, do not pass Go, do not collect $200") {
      space = 11
    }
    else if (rand_c == "Advance to Boardwalk") {
      space = 40
    }
    else if (rand_c == "Advance to Go(collect $200)") {
      space = 1
    }
    else if (rand_c == "Advance to Illinois Avenue. If you pass Go, collect $200") {
      space = 25
    }
    else if (rand_c == "Advance to St. Charles Place. If you pass Go, collect $200") {
      space = 12
    }
    else if (rand_c == "Advance to the nearest Railroad. If unowned, you may buy it from the Bank. If owned, pay owner twice the rental to which they are otherwise entitled") {
      if (space > 1 && space < 6) {
        space = 6
      }
    ...
    else if (rand_c == "Advance token to the nearest Utility. If unowned, you may buy it from the Bank. If owned, throw dice and pay owner a total ten times amount thrown.") {
      if (space > 1 && space < 13) {
        space = 13
      }
    ...
    else if (rand_c == "Go back 3 spaces") {
      space = space - 3
      if(space < 1){
        space = space + 40
      }
    }
    else if (rand_c == "Take a trip to Reading Railroad. If you pass Go, collect $200") {
      space = 6
    }
  }
  else {
  ...
  r_pos = match(rand_c, chance_cards[,1])
  assign("player_df", player_df, .GlobalEnv)
  assign("mono_board", mono_board, .GlobalEnv)
  assign("mono_graph_data", mono_graph_data, .GlobalEnv)
  return(c(space, r_pos))
...
1. mono_graph_data should have multiple rows for one player when a player takes more than one turn(rolls doubles)
2. Fix pay each player $50, so the other players actually get money
3. Change the count graph so the colors on the graph match the physical color
4. "If you pass go, collect $200" isn't working for anything
5. Not sure if free parking will ever have more than $100 in it
6. Make "get out of jail free" card functional
7. Chance3 go back three spaces and lands on community chest3, but doesn't draw a community chest card. 
...
5. when you draw a card and move to a new space, it isn't saying you moved there in the print statement in the while loop.
6. mono_graph_data should have multiple rows for one player when a player draws a card and moves a space. Should show the card space it landed on and the new space it moved to.
7. It's counting all of the chance spaces twice in the count
8. Rename r and r1 in card functions
9. Says players are going to community chest twice when it should only be once. It does this with chance too when you draw a card and you don't move to a new space. 
